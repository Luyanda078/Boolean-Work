import React from "react";
import Movies from "../Pages/myMovies";
const movies = [
    {
      id: 0,
      title: "The Quantum Paradox",
      director: "Aria Novak",
      year: 2023,
      genre: "Sci-Fi",
      rating: 8.7,
      description: "A mind-bending journey through parallel universes as a physicist tries to find her way home.",
      imageUrl: "https://images.pexels.com/photos/2531237/pexels-photo-2531237.jpeg"
    },
    {
      id: 1,
      title: "Whispers in the Wind",
      director: "Leo Chen",
      year: 2022,
      genre: "Drama",
      rating: 7.9,
      description: "A poignant tale of three generations of women in rural China facing societal changes.",
      imageUrl: "https://images.pexels.com/photos/17615502/pexels-photo-17615502/free-photo-of-chinese-woman-in-traditional-clothes-in-forbidden-city.jpeg"
    },
    {
      id: 2,
      title: "Neon Nights",
      director: "Zoe Rodriguez",
      year: 2024,
      genre: "Action",
      rating: 8.2,
      description: "A high-octane cyberpunk thriller set in a futuristic metropolis where reality is just a neural implant away.",
      imageUrl: "https://images.pexels.com/photos/3913025/pexels-photo-3913025.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    },
    {
      id: 3,
      title: "The Last Bookmark",
      director: "Ethan Blackwood",
      year: 2021,
      genre: "Mystery",
      rating: 7.5,
      description: "A bookstore owner becomes entangled in a century-old mystery when he discovers a cryptic message in an antique novel.",
      imageUrl: "https://images.pexels.com/photos/3494806/pexels-photo-3494806.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    },
    {
      id: 4,
      title: "Echoes of Eternity",
      director: "Samantha Lee",
      year: 2025,
      genre: "Fantasy",
      rating: 8.9,
      description: "An epic tale of a young bard who must use the power of music to prevent an ancient evil from awakening.",
      imageUrl: "https://images.pexels.com/photos/4090177/pexels-photo-4090177.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    },
    {
      id: 5,
      title: "The Culinary Canvas",
      director: "Marco Rossi",
      year: 2020,
      genre: "Comedy",
      rating: 7.8,
      description: "A lighthearted romp through the competitive world of avant-garde cooking as a clumsy chef aims for Michelin stars.",
      imageUrl: "https://images.pexels.com/photos/3351927/pexels-photo-3351927.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    },
    {
      id: 6,
      title: "Pixel Perfect",
      director: "Yuki Tanaka",
      year: 2023,
      genre: "Animation",
      rating: 9.1,
      description: "A heartwarming story about a glitched video game character trying to find purpose in a world of perfect avatars.",
      imageUrl: "https://images.pexels.com/photos/3800795/pexels-photo-3800795.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    },
    {
      id: 7,
      title: "Shadows of the Past",
      director: "Isabella Cortez",
      year: 2022,
      genre: "Thriller",
      rating: 8.4,
      description: "A retired detective is forced to confront her darkest case when a copycat killer emerges after 20 years.",
      imageUrl: "https://images.pexels.com/photos/7319346/pexels-photo-7319346.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    },
    {
      id: 8,
      title: "Beyond the Horizon",
      director: "Ahmed Hassan",
      year: 2024,
      genre: "Adventure",
      rating: 8.6,
      description: "A gripping account of the first manned mission to Mars and the unexpected discoveries that await the crew.",
      imageUrl: "https://images.pexels.com/photos/5259414/pexels-photo-5259414.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    },
    {
      id: 9,
      title: "The Rhythm of Life",
      director: "Grace Okafor",
      year: 2021,
      genre: "Documentary",
      rating: 9.3,
      description: "An intimate look at how music shapes cultures and brings people together across the globe.",
      imageUrl: "https://images.pexels.com/photos/17033554/pexels-photo-17033554/free-photo-of-a-group-of-women-in-colorful-dresses-and-baskets.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
    }
  ];
  
  export default movies;